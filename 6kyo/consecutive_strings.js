...
function longestConsec(strarr, k) {
  ...
  if (k <= 0 || k > strLength || !strLength) return '';

  const arrayOfConnectedStrings = [];
  const quantityOfNewStrings = strLength - k + 1;

  for (let i = 0; i < quantityOfNewStrings; i++) {
    let newString = '';
    for (let j = i; j < k + i; j++) {
      newString += strarr[j];
    }
    arrayOfConnectedStrings.push(newString);
  }

  return arrayOfConnectedStrings.reduce((acc, world) => {
    acc = acc.length < world.length ? world : acc;
    return acc;
  });
}
